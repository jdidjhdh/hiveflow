...
try:
    ...
    from .scheduler import SchedulerConfig, InProcessScheduler, GlobalLoadAwareStrategy
    from .blackboard import (
        MemoryBlackboard, TTLMemoryBlackboard, RedisBlackboard,
        SecureBlackboard, EncryptedBlackboard, KeyProvider, EnvKeyProvider, FileKeyProvider
    )
...
class HiveFlow:
    def __init__(self, config: HiveFlowConfig = None):
        ...
        if self.config.blackboard_type == "redis":
            ...
            inner = MemoryBlackboard() if not self.config.default_ttl else TTLMemoryBlackboard(default_ttl=self.config.default_ttl)
            base_bb = EncryptedBlackboard(inner, self.config.encryption_key_provider,
                                         use_compression=self.config.encrypt_compression)
        else:
            ...
            base_bb = MemoryBlackboard() if not self.config.default_ttl else TTLMemoryBlackboard(default_ttl=self.config.default_ttl)
        ...
        self.cell = Cell(self.bus, self.blackboard, self.scheduler, self.validation_pipeline,
                        default_max_queue_size=self.config.worker_max_queue_size)
        self.dag_orchestrator = DAGOrchestrator(self.blackboard)
        self.dynamic_orchestrator = DynamicOrchestrator(self.blackboard)
        self._handler_registry: Dict[str, Callable] = {}
        self._custom_strategy: Optional[Any] = None
    ...
    async def create_agent(self, agent_id: str, skills: Set[str], read_keys: Set[str],
                           write_keys: Set[str], task_handler: Callable,
                           max_queue_size: Optional[int] = None) -> Worker:
        self.register_agent_handler(agent_id, task_handler)
        return await self.cell.create_worker(agent_id, skills, read_keys, write_keys, task_handler,
    ...
    async def save_state(self):
        ...
        state = {"caps": [cap.__dict__ for cap in caps], "handler_names": handler_info, "version": 2}
        await self.blackboard.sys_put("__hiveflow_state__:agents", json.dumps(state, default=str))
    ...
    async def restore_state(self):
        try:
            ...
            for cap_dict in caps_data:
                ...
                if not handler:
                    logger.error(f"Cannot restore agent {agent_id}: no handler registered")
                    continue
                expected_name = stored_handlers.get(agent_id)
                if expected_name and handler.__name__ != expected_name:
                    logger.warning(f"Agent {agent_id} handler changed from '{expected_name}' to '{handler.__name__}'")
                restored_max_queue = cap_dict.get("max_queue_size", self.config.worker_max_queue_size)
                await self.cell.create_worker(
                    agent_id=agent_id,
                    skills=set(cap_dict["skills"]),
                    read_keys=set(cap_dict["read_keys"]),
                    write_keys=set(cap_dict["write_keys"]),
